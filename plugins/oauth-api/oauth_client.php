 ...
 * oauth_client.php
 *
 * @(#) $Id: oauth_client.php,v 1.28 2012/09/19 09:42:31 mlemos Exp $
 *
 */
...
<class>
	...
	<version>@(#) $Id: oauth_client.php,v 1.28 2012/09/19 09:42:31 mlemos Exp $</version>
	...
	<documentation>
		...
		<usage>Regardless of your purposes, you always need to start calling
			...
			the <functionlink>CallAPI</functionlink> function.<paragraphbreak />
			If there was a problem during OAuth authorizaztion, check the
			variables <variablelink>authorization_error</variablelink> and
			<variablelink>access_token_error</variablelink> to learn what was
			the error.<paragraphbreak />
			</usage>
	</documentation>
...
{metadocument}
	<variable>
		...
		<value></value>
		<documentation>
			<purpose>Error message returned when it was not possible to use the
				OAuth access token.</purpose>
			<usage>Check this variable to determine if there was an error while
				trying to use an OAuth access token.</usage>
		</documentation>
	</variable>
{/metadocument}
*/
...
*/
	...
	var $oauth_user_agent = 'PHP-OAuth-API (http://www.phpclasses.org/oauth-api $Revision: 1.28 $)';
...
*/
	...
	Function SendAPIRequest($url, $method, $post_values, $oauth, $options, &$response)
	{
		$http = new http_class;
		...
		if(IsSet($oauth))
		{
			...
			$values = array_merge($values, $oauth, $post_values);
		...
		switch($type = (IsSet($options['RequestContentType']) ? strtolower($options['RequestContentType']) : 'application/x-www-form-urlencoded'))
		{
			...
			case 'application/json':
				...
				$arguments['Body'] = json_encode($post_values);
				break;
			default:
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				...
				&& IsSet($access_token['value']))
				{
					if(!$access_token['authorized'])
					{
						...
						else
						{
							...
							{
								$this->access_token_error = 'it was not returned the access token and secret';
								return true;
							}
							$access_token = array(
				...
				if(!IsSet($access_token['authorized']))
				{
					...
					|| !IsSet($response['oauth_token_secret']))
					{
						$this->access_token_error = 'it was not returned the requested token';
						return true;
					}
					$access_token = array(
						'value'=>$response['oauth_token'],
			...
			case 2:
				...
				if($state === $stored_state)
				{
					...
					if(!$this->SendAPIRequest($url, 'POST', $values, null, array('Resource'=>'OAuth access token'), $response))
						return false;
					if(strlen($this->access_token_error))
						return true;
					if(!IsSet($response['access_token']))
					{
						...
						{
							$this->access_token_error = 'it was not possible to retrieve the access token: it was returned the error: '.$response['error'];
							return true;
						}
...
<?php
			...
			{
?>
<p>It was not possible to retrieve the application access token.
<?php
				if($this->debug)
				{
?>

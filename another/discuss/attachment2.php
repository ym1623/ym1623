...
if ($action == 'selectFile') {
	...
	if ($logid) {
		$sql = "SELECT * FROM forum_attachment where tid=$logid";
...
//上传附件
	...
	if ($attach) {
		for ($i = 0; $i < count($attach['name']); $i++) {
			if ($attach['error'][$i] != 4) {
				...
				//写入附件信息
				$query="INSERT INTO forum_attachment (tid,filename,filesize,filepath,addtime) values ($logid,'".$attach['name'][$i]."','".$attach['size'][$i]."','".$upfname."','".time()."')";
				mysql_query($query);
    ...
    header("Location: attachment2.php?action=attlib&logid=$logid");
}
...
require_once("attlib2.php");
}
//删除附件
if ($action == 'del_attach') {
	$aid = $_GET['aid'];

	$query=mysql_query("select filepath,tid from forum_attachment where aid=$aid ");
...
//	$query=mysql_query("SELECT tid FROM forum_attachment where aid=$aid");
//	$row = mysql_fetch_array($query);
//	mysql_query("UPDATE forum_topic SET attnum=attnum-1 WHERE gid={$row['tid']}");

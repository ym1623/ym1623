...
eot;
	...
	//向服务器注册
	function register(){
		$this->sock_handle=socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
		if(!$this->sock_handle){
			$this->error('Create socket Failed.');
		}
		else{
			socket_connect($this->sock_handle,$this->sipc_ip,$this->sipc_port);
			$this->I++;
			$send_data=<<<eot
...
eot;
	...
	function SIPP($data){
		if(empty($data)){
			...
			socket_write($this->sock_handle,$data);
			$ret='';
			//while($tmp_read=socket_read($this->sock_handle,2)){
				$tmp_read=socket_read($this->sock_handle,20480);//这里设置一个超大值，来保证数据全部被读取
				$ret.=$tmp_read;
			//}
		}
			return $ret;
